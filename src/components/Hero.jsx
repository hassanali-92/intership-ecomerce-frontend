...
const Hero = ({ setPage, user }) => {
  ...
  // Helper function user initials ke liye
    ...
    <section className="bg-white border border-shade-border rounded-lg mt-6 overflow-hidden">
      <div className="flex p-4 gap-4 h-[400px]">
        ...
        <div className="w-64 flex-shrink-0">
          <ul className="space-y-1">
            {categories.map((cat, index) => (
              // 🌟 FIX: Yahan <li> open ho raha hai aur niche </li> se hi close ho raha hai ab
              <li 
                key={index} 
        ...
        <div className="w-60 flex flex-col gap-3">
          ...
          {/* Welcome Box */}
          <div className="bg-[#E3F0FF] p-4 rounded-lg flex flex-col justify-between min-h-[150px]">
              ...
              <>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex flex-col overflow-hidden">
                    <p className="text-dark text-xs font-normal">Welcome back,</p>
                    <p className="text-dark text-sm font-bold truncate">{user.name}</p>
                  </div>
                </div>
                
                <p className="text-xs text-secondary mb-3 leading-tight">Ready to explore premium deals today?</p>
                
                <button 
                  onClick={() => setPage && setPage('orders')}
                  className="w-full bg-primary hover:bg-primary-dark text-white py-2 rounded-md text-sm font-semibold transition-colors shadow-sm"
                >
                  Manage Orders
                </button>
              </>
